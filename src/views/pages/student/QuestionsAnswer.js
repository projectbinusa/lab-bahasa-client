...
function QuestionsAnswer() {
  ...
  const isThinkTimeReady = (thinkTime) => {
    try {
      ...
      const thinkTimeDate = new Date(thinkTime);
  ...
  const renderAction = (question) => {
    if (!isThinkTimeReady(question.think_time)) {
      ...
      return question.name === "Pilihan Ganda" ? (
        <Link
          to={`/jawab-soal-pilihan-ganda/${class_id}/id/${question.id}`}
      ...
      return (
        <button
          className="py-2 px-3 bg-red-500 whitespace-nowrap rounded-lg text-white"
          disabled>
          Waktu Habis
        </button>
      );
  ...
  return (
    <div className="flex flex-col h-screen overflow-x-hidden">
      ...
      <div className="px-4 sm:px-8 md:px-16 lg:px-32">
        <div className="w-full p-4 text-left bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700 mt-8">
