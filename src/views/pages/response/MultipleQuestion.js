import React, { useEffect, useState } from "react";
import axios from "axios";
import Swal from "sweetalert2";
import { API_DUMMY } from "../../../utils/api";
import Navbar from "../../../component/Navbar1";
import { Link } from "react-router-dom";

const authConfig = {
  headers: {
    "auth-event": `jwt ${localStorage.getItem("token")}`,
  },
};

function MultipleQuestion() {
  const [name, setName] = useState("Pilihan Ganda");
  const [type, setType] = useState("");
  const [thinkTime, setThinkTime] = useState("");
  const [multiple_questions_id, setMultiple_questions_id] = useState([]);
  const [list, setList] = useState([]);
  const [answerTime, setAnswerTime] = useState("");
  const [timeLeft, setTimeLeft] = useState(null);
  const [phase, setPhase] = useState("think");
  const [timer, setTimer] = useState(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [limit, setLimit] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const class_id = localStorage.getItem("class_id");

  const parseTimeToSeconds = (time) => {
    const [hours, minutes] = time.split(":").map(Number);
    const now = new Date();
    const target = new Date();

    target.setHours(hours, minutes, 0, 0);

    if (target < now) {
      target.setDate(target.getDate() + 1);
    }

    return Math.floor((target - now) / 1000);
  };

  const saveChange = async () => {
    const data = {
      name: name,
      multiple_questions_id: multiple_questions_id,
      type: type,
      think_time: thinkTime,
      answer_time: answerTime,
    };
    let url_hit = `${API_DUMMY}/api/instructur/class/${class_id}/response_competition_multiple_questions`;

    try {
      const response = await axios.post(url_hit, data, authConfig);
      if (response.status === 200) {
        Swal.fire({
          icon: "success",
          title: "Kompetisi dimulai.",
          showConfirmButton: false,
          timer: 1500,
        });
        getAllData();
      }
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Gagal Menambahkan Data.",
        showConfirmButton: false,
        timer: 1500,
      });
    }
  };

  const startTimer = (duration, phase) => {
    clearInterval(timer);
    setTimeLeft(duration);
    const newTimer = setInterval(() => {
      setTimeLeft((prevTime) => {
        if (prevTime <= 0) {
          clearInterval(newTimer);
          if (phase === "think") {
            setPhase("thinkTimeHabis");
          } else if (phase === "answer") {
            setPhase("answerTimeHabis");
          }
          return 0;
        }
        return prevTime - 1;
      });
    }, 1000);
    setTimer(newTimer);
  };

  useEffect(() => {
    getAllData();
  }, [currentPage, limit, searchTerm]);

  const startCompetition = () => {
    setPhase("think");
    const thinkTimeSeconds = parseTimeToSeconds(thinkTime);
    const answerTimeSeconds = parseTimeToSeconds(answerTime);
    startTimer(thinkTimeSeconds, "think");
    setTimeout(() => {
      setPhase("answer");
      startTimer(answerTimeSeconds, "answer");
    }, thinkTimeSeconds * 1000);
    saveChange().then(() => {
      //   window.location.reload("/question");
      console.log("Berhasil");
    });
  };

  const getInstructions = () => {
    switch (type) {
      case "First to Answer":
        return "Kompetisi di mana siswa diberikan pertanyaan dan mereka berlomba untuk menjadi orang pertama yang memberikan jawaban yang benar.";
      case "Enter an Answer":
        return "Siswa memasukkan jawaban mereka ke dalam sistem. Jawaban ini kemudian dapat dinilai oleh guru.";
      case "Demo to Answer":
        return "Tekan tombol 'start' untuk memulai menjawab, hanya siswa pertama yang menekan tombol jawab yang dapat mendemonstrasikan layar kepada siswa lainnya.";
      default:
        return "";
    }
  };

  const getAllData = async () => {
    try {
      const response = await axios.get(
        `${API_DUMMY}/api/instructur/class/${class_id}/multiple_choice_questions?limit=${limit}&name=${searchTerm}&page=${currentPage}`,
        authConfig
      );
      setList(response.data.data);
      setTotalPages(response.data.pagination.total_page);
    } catch (error) {
      console.log(error);
    }
  };

  const toggleQuestionSelection = (id) => {
    setMultiple_questions_id((prevSelected) => {
      if (prevSelected.includes(id)) {
        return prevSelected.filter((qid) => qid !== id);
      } else {
        return [...prevSelected, id];
      }
    });
  };

  const indexToLetter = (index) => {
    return String.fromCharCode(65 + index);
  };

  return (
    <div className="flex flex-col min-h-screen h-max from-slate-100 bg-gradient-to-b to-blue-200">
      <Navbar />
      <div className="content-page container mx-auto mt-10">
        <div className="p-3 flex justify-around">
          <div className="w-full">
            <div className="flex gap-4">
              <h2 className="font-bold text-xl bg-blue-700 w-fit rounded-lg text-white px-2 mb-4">
                Soal Pilihan Ganda
              </h2>
              <Link
                to={"/list-soal-pilihan-ganda/" + class_id}
                className="font-bold text-xl bg-blue-700 w-fit rounded-lg text-white px-2 mb-4">
                Buat Soal
              </Link>
            </div>
            <hr />
            {list.map((data, index) => (
              <div className="flex gap-3 p-4" key={data.id}>
                <div>
                  <button
                    onClick={() => toggleQuestionSelection(data.id)}
                    className={`p-2 w-11 rounded-md text-white ${
                      multiple_questions_id.includes(data.id)
                        ? "bg-blue-700"
                        : "bg-blue-700"
                    } border-2 border-blue-700 h-fit`}>
                    {multiple_questions_id.includes(data.id) ? (
                      <i className="fa-solid fa-check"></i>
                    ) : (
                      index + 1
                    )}
                  </button>
                </div>
                <div>
                  <p>{data.question_text}</p>
                  {data.options.map((option, optIndex) => (
                    <div
                      className="flex items-center mb-2 gap-3 mt-3"
                      key={optIndex}>
                      <input
                        type="radio"
                        checked={optIndex === data.correct_answer}
                        name={`question-${data.id}`}
                        className="ml-2"
                        readOnly
                      />
                      <div className="flex gap-1">
                        <p>
                          {" "}
                          {indexToLetter(optIndex)}. {option}
                        </p>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            ))}
          </div>
          <div className="w-11/12 p-3 bg-gray-100 rounded-lg shadow-lg border border-gray-300 mx-auto h-fit">
            <h1 className="text-lg sm:text-xl font-bold text-gray-800">
              Kompetisi Respon
            </h1>
            <div className="mb-3 mt-3">
              <label
                htmlFor="type"
                className="mb-1 text-sm font-semibold text-gray-700 block">
                Jenis:
              </label>
              <select
                id="type"
                value={type}
                onChange={(e) => setType(e.target.value)}
                className="block w-full p-2 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-700 focus:border-blue-700">
                <option value="">Pilih jenis</option>
                <option value="First to Answer">Tinjau untuk menjawab</option>
                <option value="Enter an Answer">Masukkan jawaban</option>
              </select>
            </div>
            <div className="mb-3">
              <label
                htmlFor="think-time"
                className="mb-1 text-sm font-semibold text-gray-700 block">
                Waktu berpikir:
              </label>
              <input
                type="datetime-local"
                id="think-time"
                value={thinkTime}
                onChange={(e) => setThinkTime(e.target.value)}
                className="block w-full p-2 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-700 focus:border-blue-700"
              />
            </div>
            <div className="mb-3">
              <label
                htmlFor="answer-time"
                className="mb-1 text-sm font-semibold text-gray-700 block">
                Waktu jawab:
              </label>
              <input
                type="datetime-local"
                id="answer-time"
                value={answerTime}
                onChange={(e) => setAnswerTime(e.target.value)}
                className="block w-full p-2 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-700 focus:border-blue-700"
              />
            </div>
            <div className="mb-3 w-[70%]">
              <p className="text-sm font-semibold text-gray-700">
                Instructions:
              </p>
              <p className="text-gray-700">{getInstructions()}</p>
            </div>
            <button
              type="submit"
              className="w-full py-2 font-semibold text-white bg-blue-700 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-700"
              onClick={startCompetition}>
              Mulai Kompetisi
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

export default MultipleQuestion;
